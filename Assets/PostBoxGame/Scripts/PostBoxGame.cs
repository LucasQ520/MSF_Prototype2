...
namespace PostBoxGame
{
    public class PostBoxGame : MonoBehaviour
    {
        ...
        float feedbackTime;
        Texture2D white;
        GUIStyle heading,body,small,mailText,button,buttonDark,center,centerDark,bodyDark,smallDark,headingDark,right;
        readonly Color ink=new Color(.075f,.15f,.18f),cream=new Color(.96f,.94f,.87f),gold=new Color(.91f,.70f,.31f);
        readonly Color panel=new Color(.13f,.25f,.28f),line=new Color(.29f,.43f,.43f),coral=new Color(.78f,.33f,.24f);
        ...
        void Awake()
        {
            ...
            if(Camera.main==null)
            {
                ...
                camera.backgroundColor=new Color(.055f,.10f,.14f);camera.cullingMask=0;
            }
        ...
        void Update()
        {
            ...
            if(tracked)
            {
                tiltX=Mathf.Clamp((.5f-tracker.center.x)*2.5f,-1,1);
                tiltY=Mathf.Clamp((tracker.center.y-.5f)*2.5f,-1,1);
            ...
            if(active!=null)
            {
                if(stepCooldown<=0)
                {
                    int dx=Mathf.Abs(tiltX)>.45f?(tiltX>0?1:-1):0;
                    int dy=Mathf.Abs(tiltY)>.45f?(tiltY>0?-1:1):0;
                    if(dx!=0 || dy!=0){TryMove(dx,dy);stepCooldown=.18f;}
                }
        ...
        bool Fits(MailPiece p,int x,int y)
        {
            ...
            foreach(var q in stored)if(q!=p && x<q.x+q.Width && x+p.Width>q.x && y<q.y+q.Height && y+p.Height>q.y)return false;
            return true;
        }
        void ActivateNext()
        {
        ...
        void TryMove(int dx,int dy)
        {
            ...
            else if(Fits(active,active.x+dx,active.y)){active.x+=dx;}
            else if(Fits(active,active.x,active.y+dy)){active.y+=dy;}
        }
        void Rotate()
        {
        ...
        void InitStyles()
        {
            ...
            small=new GUIStyle(body){fontSize=15};small.normal.textColor=new Color(.78f,.84f,.78f);
        ...
        bool Button(Rect r,string s,bool selected=false,bool primary=false)
        {
            ...
            Box(new Rect(r.x,r.y+3,r.width,r.height),new Color(.03f,.08f,.09f,.65f));
            Box(r,primary?gold:selected?cream:hover?new Color(.25f,.42f,.43f):panel);
        ...
        {
            Box(new Rect(0,0,1280,103),panel);Box(new Rect(0,100,1280,3),coral);
            ...
            Label(new Rect(650,155,520,60),"FIND YOUR BALANCE",heading);
            ...
            if(cams.Length>0 && Button(new Rect(660,435,220,48),"CAMERA: "+(camIndex+1)+" / "+cams.Length)){camIndex=(camIndex+1)%cams.Length;tracker.StartCamera(camIndex);}
        ...
        void DrawGame()
        {
            ...
            Box(new Rect(0,0,1280,90),panel);Box(new Rect(0,88,1280,3),coral);
            Box(new Rect(410,0,460,18),slotOpen?gold:new Color(.38f,.42f,.39f));
            ...
            Box(new Rect(277,119,726,534),new Color(.07f,.14f,.17f));
            float angle=tiltX*3;GUIUtility.RotateAroundPivot(angle,new Vector2(640,390));
            for(int y=0;y<config.rows;y++)for(int x=0;x<config.columns;x++)
            {
                var r=new Rect(gx+x*cw,gy+y*ch,cw-3,ch-3);
                Box(r,new Color(.17f,.27f,.29f));
            }
            foreach(var p in stored)DrawPiece(p,false);
            if(active!=null)DrawPiece(active,true);
            GUIUtility.RotateAroundPivot(-angle,new Vector2(640,390));
            int occupied=0;foreach(var p in stored)occupied+=p.Width*p.Height;
            ...
            if(keyboardMode)Label(new Rect(25,475,230,100),"DEBUG CONTROL\nArrow keys tilt • Shift closes slot",small);
        ...
        void DrawPiece(MailPiece p,bool selected)
        {
            ...
            Color c=p.spec.kind==MailKind.Parcel?new Color(.71f,.55f,.34f):p.spec.kind==MailKind.Magazine?new Color(.65f,.78f,.71f):cream;
            Box(r,selected?gold:c);
            Box(new Rect(r.x+5,r.y+5,r.width-10,4),coral);
        ...
        void DrawBooklet()
        {
            ...
            Label(new Rect(325,150,630,50),"MERCER STREET / RESIDENT CARD",headingDark);
            Box(new Rect(334,207,620,2),coral);
        ...
        void DrawResults()
        {
            ...
            {
                float y=205+i*41;
                if(i%2==0)Box(new Rect(290,y,700,39),new Color(.18f,.31f,.33f));
                Label(new Rect(310,y,470,39),labels[i],body);
                Label(new Rect(810,y,155,39),values[i],right);
            }
